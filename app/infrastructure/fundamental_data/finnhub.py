...
class FinnhubFundamentalDataProvider:
    ...
    The provider intentionally requests standardized annual income statements
    and balance sheets. It does not calculate financial metrics; the domain
    analyzers own those rules.
    """
    ...
    def get_periods(
        ...
        periods = self._merge_periods(
            income.get("financials", []),
            balance.get("financials", []),
            as_of,
        )
    ...
    def _merge_periods(
        ...
        balance_by_period = {
            cls._period_date(row): row
            for row in balance_rows
            if cls._period_date(row) is not None
        }
            ...
            merged.append(
                FinancialPeriod(
                    ...
                    current_assets=cls._decimal(
                        balance_row.get("totalCurrentAssets")
                    ),
                    current_liabilities=cls._decimal(
                        balance_row.get("totalCurrentLiabilities")
                    ),
                )
            )
