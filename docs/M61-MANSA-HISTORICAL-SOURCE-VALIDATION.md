...
- A separate Fundamentals Suite, but the published methodology/example does not establish Egyptian point-in-time financial-snapshot coverage required by M61.
...
Mansa market data can proceed toward M61 dataset acceptance only if all of these are evidenced:
...
- the resulting transformed artifact passes the existing M61 dataset validator.

## Important licensing boundary

Mansa's licensing page distinguishes application caching from redistribution. Current documentation allows caching within tier-specific windows and states that raw-data redistribution requires Institutional licensing.

M61 must therefore obtain explicit confirmation from Mansa before treating API responses as a long-lived immutable research archive. Internal research/backtesting storage is not assumed to be prohibited, but it is also not inferred to be permitted merely from API access.
...
Mansa documents a Fundamentals Suite with fiscal-period financial figures and source-document URLs. Its current methodology says fundamentals are extracted from official company filings and human-verified before publication. However, the published coverage/examples do not establish the Egyptian point-in-time `available_at` evidence required by DEC-130.
