...
def test_api_passes_history_state_filters():
    ...
    with TestClient(app) as client:
        response = client.get(
            f"/api/v1/workflows/executions/{model.execution_id}/history"
            "?page_size=2&from_state=created&to_state=running"
        )
...
def test_api_maps_invalid_history_filter_to_400():
    query = FakeHistoryQuery(
        error=InvalidScheduledWorkflowExecutionHistoryQueryError(
            "to_state must be a valid scheduled workflow execution state"
        )
    )
    app = create_app(
        InMemoryAnalysisResultStore(),
    ...
    with TestClient(app) as client:
        response = client.get(
            f"/api/v1/workflows/executions/{uuid4()}/history?to_state=unknown"
        )
