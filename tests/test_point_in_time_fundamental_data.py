...
def snapshot(year: int, available: date, revenue: str = "100") -> HistoricalFinancialSnapshot:
...
def test_missing_point_in_time_history_fails_explicitly():
    ...
    with pytest.raises(ValueError, match="Insufficient point-in-time"):
        PointInTimeFundamentalDataProvider(source).get_periods(stock, date(2025, 3, 1))
